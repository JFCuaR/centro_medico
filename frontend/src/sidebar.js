...
const Sidebar = ({ isOpen }) => {
  ...
  // Función para mostrar enlaces normales o bloqueados
  const renderLink = (link) => {
    const bloqueado = rutasBloqueadas.includes(link.path);

    if (bloqueado) {
      return (
        <span
          key={link.path}
          className="submenu-item disabled-link"
  ...
  const renderMenuPorRol = () => {
    ...
    const secciones = {
      ...
      clinica: [
        ...
        {
          path: '/HistorialEncamaciento',
          label: '📺 Buscar paciente en encamamiento',
        },
      ],
    ...
    // Admin ve todo, pero algunas opciones están bloqueadas
    if (rol === 'admin') {
      return (
        <>
          {Object.entries(secciones).map(([key, links]) => (
    ...
    if (rol === 'farmaceutico' || rol === 'empleado') {
      return (
        <div className="menu-section">
          ...
          <div className="menu-content">
            {secciones.farmacia.map((link) => renderLink(link))}
          </div>
        </div>
      );
    }
    ...
    if (rol === 'doctor') {
      return (
        <div className="menu-section">
          ...
          <div className="menu-content">
            {secciones.clinica.map((link) => renderLink(link))}
          </div>
        </div>
      );
    }
    ...
    if (rol === 'enfermero') {
      return (
        <div className="menu-section">
          <div className="menu-title">👥 ENFERMERO</div>

          <div className="menu-content">
            {secciones.enfermero.map((link) => renderLink(link))}
          </div>
        </div>
      );
    }
